...
public class OrQueryTest {
    ...
    public void evaluateReturnsTrueIfAllSubQueriesAreTrue() {
        ...
        assertTrue(or(new TrueQuery(), new TrueQuery(), new TrueQuery()).evaluate(th, tr));
    }
    ...
    public void evaluateReturnsFalseIfAllSubQueriesAreFalse() {
        ...
        assertFalse(or(new FalseQuery(), new FalseQuery(), new FalseQuery()).evaluate(th, tr));
    }

    @Test
    public void evaluateReturnsTrueIfSomeSubQueriesAreFalse() {
        TableHeader th = header("key1", "key2");
        TableRow tr = row("foo", "bar");

        assertTrue(or(new FalseQuery(), new TrueQuery(), new TrueQuery()).evaluate(th, tr));
    }
